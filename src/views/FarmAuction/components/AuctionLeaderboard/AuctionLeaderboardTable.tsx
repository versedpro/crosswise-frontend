...
interface LeaderboardRowProps {
  bidder: Bidder
  crssPriceBusd: BigNumber
  isMobile: boolean
}

const LeaderboardRow: React.FC<LeaderboardRowProps> = ({ bidder, crssPriceBusd, isMobile }) => {
        ...
        <Flex flexDirection="column" width="100%" justifyContent="flex-end" pr={[null, null, '24px']}>
          ...
          {crssPriceBusd.gt(0) ? (
            <Text fontSize="12px" color="textSubtle" textAlign="right">
              ~${getBalanceNumber(amount.times(crssPriceBusd)).toLocaleString('en', { maximumFractionDigits: 0 })}
            </Text>
          ) : (
      ...
      <GridCell isTopPosition={isTopPosition}>
        <SubMenu component={<EllipsisIcon height="16px" width="16px" />}>
          ...
          {lpAddress && (
            <SubMenuItem
              as={LinkExternal}
              href={`http://app.crosswise.finance/pool/${lpAddress}`}
              bold={false}
              color="text"
            >
              {t('LP Info')}
    ...
    <Box>
      <LeaderboardContainer>
        ...
        {bidders.slice(0, visibleBidders).map((bidder) => (
          <LeaderboardRow key={bidder.account} bidder={bidder} crssPriceBusd={crssPriceBusd} isMobile={isXs || isSm} />
        ))}
      </LeaderboardContainer>
