...
const LotteryCardContent = () => {
  ...
  const crssPriceBusdAsString = usePriceCrssBusd().toString()

  const cakePrizesText = t('%cakePrizeInUsd% in CAKE prizes this round', { cakePrizeInUsd: crssPriceBusdAsString })
  const [pretext, prizesThisRound] = cakePrizesText.split(crssPriceBusdAsString)
  ...
  useEffect(() => {
    ...
    const fetchCurrentLotteryPrize = async () => {
      ...
      const prizeInBusd = crssPriceBusd.times(amountCollectedInCake)
  ...
  }, [lotteryId, slowRefresh, setCurrentLotteryPrize, crssPriceBusd])
